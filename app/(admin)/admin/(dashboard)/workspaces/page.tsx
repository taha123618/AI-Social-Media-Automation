...
import { getBusinesses, deleteBusiness } from "../../actions/admin.actions";
import { Button } from "@/components/ui/button";
import { DataTable } from "../../_components/data-table";
import { businessColumns } from "../../_components/columns";
import {
   Dialog,
   DialogContent,
   DialogDescription,
...
import { Building2, RefreshCw, Shield, Search, Loader2, Trash2, Plus } from "lucide-react";
      ...
      <div ref={containerRef} className="space-y-10 pb-10">
         <div className="header-section flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="space-y-1">
               <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-4xl font-extrabold tracking-tight text-foreground drop-shadow-sm">Workspace Registry</h1>
               </div>
               <p className="text-muted-foreground text-lg font-medium ml-15">Manage business entities, organizational nodes, and workspace resources.</p>
            </div>
            <div className="flex items-center gap-3">
               <Button variant="outline" onClick={fetchBusinesses} className="glass-card hover:bg-muted font-semibold px-6 border-border" disabled={isLoading}>
               ...
               <Button asChild className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/20 font-bold px-8 rounded-xl transition-all hover:scale-105 active:scale-95">
                  <Link href="/admin/workspaces/add">
               ...
               {isLoading ? (
                  ...
                  >
                     ...
                     <div className="text-center space-y-2">
                        <p className="text-xl font-bold text-foreground">Mapping Workspace Grid...</p>
                        <p className="text-sm font-medium text-muted-foreground animate-pulse tracking-widest uppercase">Synchronizing Cluster States</p>
                     </div>
                  </motion.div>
               ) : (
                  <motion.div
                  ...
                  >
                     <DataTable
                        columns={businessColumns(setDeleteId)}
                        data={businesses}
                        searchKey="name"
                        searchPlaceholder="Search clusters by name or slug..."
                     />
                  </motion.div>
               )}
            </AnimatePresence>
         ...
         <Dialog open={!!deleteId} onOpenChange={(open: any) => !open && setDeleteId(null)}>
            <DialogContent className="glass-card border-border bg-background/95 backdrop-blur-2xl">
               <DialogHeader>
                  <DialogTitle className="text-2xl font-bold text-foreground">Irreversible Node Decommission</DialogTitle>
                  <DialogDescription className="text-muted-foreground text-base">
                     You are about to decommission this workspace node. All associated data, social accounts, and workflows will be permanently erased. This action is logged.
                  </DialogDescription>
               </DialogHeader>
