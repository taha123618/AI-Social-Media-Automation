...
export async function GET(request: Request) {
  try {
    ...
    const messages = await SocialMediaService.getMessages(businessId, Platform.LINKEDIN, threadId);
...
export async function POST(request: Request) {
  try {
    ...
    const messageId = await SocialMediaService.sendMessage(businessId, Platform.LINKEDIN, recipientId, message);
