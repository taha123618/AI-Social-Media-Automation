...
export async function GET(request: Request) {
  try {
    ...
    const messages = await SocialMediaService.getMessages(businessId, Platform.INSTAGRAM, threadId);
    return NextResponse.json({ success: true, messages });
  } catch (error) {
    console.error('Instagram Messages GET Error:', error);
...
export async function POST(request: Request) {
  try {
    ...
    const messageId = await SocialMediaService.sendMessage(businessId, Platform.INSTAGRAM, recipientId, message);
    return NextResponse.json({ success: true, messageId });
  } catch (error) {
    console.error('Instagram Messages POST Error:', error);
