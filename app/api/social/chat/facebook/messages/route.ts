...
export async function GET(request: Request) {
  try {
    ...
    const messages = await SocialMediaService.getMessages(businessId, Platform.FACEBOOK, threadId);
    return NextResponse.json({ success: true, messages });
  } catch (error) {
    console.error('Facebook Messages GET Error:', error);
...
export async function POST(request: Request) {
  try {
    ...
    const messageId = await SocialMediaService.sendMessage(businessId, Platform.FACEBOOK, recipientId, message);
    return NextResponse.json({ success: true, messageId });
  } catch (error) {
    console.error('Facebook Messages POST Error:', error);
