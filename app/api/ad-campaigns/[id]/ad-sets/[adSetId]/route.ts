...
export async function PATCH(
  ...
  try {
    ...
    const updated = await prisma.adSet.update({ where: { id: adSetId }, data });
    return NextResponse.json({ success: true, adSet: updated });
  } catch (error: any) {
    console.error('[PATCH /api/ad-campaigns/[id]/ad-sets/[adSetId]]', error);
    return NextResponse.json({ success: false, error: 'Failed to update ad set' }, { status: 500 });
  }
}
