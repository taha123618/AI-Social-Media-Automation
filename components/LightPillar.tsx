...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    console.log('LightPillar component mounted');
    if (!containerRef.current) {
      console.error('Container ref is null');
      return;
    }

    console.log('Initializing LightPillar...');

    const container = containerRef.current;
    const width = container.clientWidth;
    const height = container.clientHeight;

    // Check device capabilities on client side only
  ...
  if (!webGLSupported) {
    ...
    return (
      <div
        ...
        style={{ mixBlendMode }}
      >
        WebGL not supported - LightPillar cannot render
      </div>
    );
  }
